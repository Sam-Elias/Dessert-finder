...
class App extends Component {
  ...
  getUsers = () => {
    fetch("https://api.myjson.com/bins/cm76u")
      ...
      .then(data => {
        this.setState(
          { allUsers: data.users, currentUsers: data.users },
          this.loadMap()
        );
      }) //sets the state when it receives the data from the promise, then it calls the loadMap function
      .catch(error => {
        return (
          <div>
            <h1>We got this problem :${error}</h1>
  ...
  loadMap = () => {
    loadScript(
      "https://maps.googleapis.com/maps/api/js?key=AIzaSyARW8_P5byg0dhnLlZkraFBi6X_PmMshQQ&v=3&callback=renderMap"
    );
  ...
  renderMap = () => {
    const map = new window.google.maps.Map(document.getElementById("map"), {
      center: { lat: 34.42083, lng: -119.698189 },
      zoom: 13
    });
    console.log("renderMap ran");
  ...
  makeMarkers = map => {
    ...
    this.setState(
      { markers: _markers },
      console.log("This shiuld be after markers set state")
    );
  };
  ...
  filterUsers = query => {
    ...
    let filteredByDessert = this.state.allUsers.filter(user =>
      match.test(user.dessert)
    ); //create an array of users.dessert that match the regular expression
    this.setState(
      { query: query.target.value.trim(), currentUsers: filteredByDessert },
      () => this.renderMap()
    ); //set the query state and update current users
  };

  handleClick = clicked => {
    this.state.markers.forEach(marker =>
      this.closeInfoWindow(marker.infowindow)
    );
    let liValue;
    let marker;
    if (clicked.target) {
      liValue = clicked;
      const matchedMarker = this.state.markers //loop through the markers array and match with the li clicked
        .find(marker => marker.title === liValue.target.innerHTML);
      const matchedUser = this.state.currentUsers //loop through the users array and match with the dessert clicked
